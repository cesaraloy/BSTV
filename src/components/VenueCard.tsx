...
export default function VenueCard({ venue, onClick, compact }: Props) {
  return (
    ...
    >
      {/* Image placeholder */}
      <div className="w-14 h-14 rounded-xl bg-brand-accent flex items-center justify-center shrink-0 overflow-hidden">
        ...
        {!compact && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {venue.competitions.map(c => (
              <span key={c} className="text-[9px] bg-brand-accent text-brand-text px-1.5 py-0.5 rounded-full">
                {c}
              </span>
            ))}
          </div>
