...
export default function VenueDetailScreen({ venue, onBack, onMatchClick }: Props) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-6">
      {/* Hero image */}
      <div className="relative h-52 bg-gradient-to-b from-brand-accent to-brand-surface flex items-center justify-center">
        ...
        <div className={`absolute bottom-4 right-4 px-3 py-1.5 rounded-full text-xs font-bold ${
          venue.is_open ? 'bg-brand-green/90 text-brand-bg' : 'bg-red-500/90 text-white'
        }`}>
      ...
      <div className="mx-5 -mt-6 relative z-10">
        <div className="bg-brand-card border border-brand-border rounded-2xl p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button className="flex items-center justify-center gap-2 py-3 bg-brand-green rounded-xl font-semibold text-sm text-brand-bg">
          ...
          <div className="flex flex-col gap-2">
            {relatedMatches.map(match => {
              ...
              return (
                ...
                >
                  <div className="w-9 h-9 rounded-xl bg-brand-accent flex items-center justify-center shrink-0">
                  ...
                  <div className="flex flex-col items-end shrink-0 gap-0.5">
                    <span className="text-xs font-bold text-brand-green">{match.match_time}</span>
                    <span className="text-[10px] text-brand-muted">{match.match_date}</span>
                    {isReminder && (
                      <span className="text-[9px] text-brand-green">🔔</span>
                    )}
                  </div>
